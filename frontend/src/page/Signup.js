...
function Signup() {
  ...
  const PostData = ()=>{
    ...
    if(!passwordREGEX.test(password)){
      setError("password should not be less than 6 characters and must contain al least one number, one uppercase, one lowercase")
      return
    }
    ...
    <div>
      <div className='login-container'>
        ...
        <br /><br/>
          ...
          <div>
            <p className='Login-Title'>Already have an account <Link to="/Login">Login</Link></p>
          </div>
        </div>
      </div>
    </div>
